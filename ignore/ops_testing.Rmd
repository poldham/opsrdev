...
```{r}
more_pizza <- ops_publications(query = "pizza", type = "ta", start = 1990, end = 2000)
```
...
```{r}
post_biblios <- POST("http://ops.epo.org/3.1/rest-services/published-data/publication/epodoc/biblio", content_type("plain/text"), accept("application/json"), body = y)
test <- content(post_biblios100)

#some kind of issue with ops_post
post_biblios <- ops_post(z)
post_biblios1 <- content(post_biblios)
test <- ops_multi_biblio(post_biblios1)
ops_multi_biblio(post_bibliosy)

ops_biblio(post_biblios)
ops_biblio(post_bibliosy)
```


Alternative approach to work out why format is different
```{r}
library(dplyr)
